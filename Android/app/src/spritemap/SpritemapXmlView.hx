...
class SpritemapXmlView extends Sprite {
    ...
    function init():Void {
        ...
        layout = XmlLayout.loadAsset("assets/ui/spritemap_to_funky.xml");
    ...
    function buildFromLayout():Void {
        ...
        if (subtitleField == null) subtitleField = makeFallbackLabel("UI controlada por XML para la pantalla principal.", 24, 52, 14, false, AppConfig.COLOR_MUTED);
        if (footerField == null) footerField = makeFallbackLabel("© 2026 Jarret Labs", 24, 740, 12, false, AppConfig.COLOR_MUTED);

        if (statusField == null) statusField = makeFallbackLabel("Listo.", 24, 740, 13, false, AppConfig.COLOR_MUTED);
    ...
    function addNode(parent:Sprite, node:Xml, ?offsetX:Float = 0, ?offsetY:Float = 0):Void {
        switch (node.nodeName) {
            ...
            case "input":
                ...
                input.setWidth(XmlLayout.attrFloat(node, "w", 320));
    ...
    function layoutUi():Void {
        ...
        if (wide) {
            ...
            var cardH = 306.0;
            layoutPanel("project", left, y, colW, cardH);
            layoutPanel("actions", left + colW + gap, y, colW, cardH);
            layoutProjectContent(colW - 36, false);
            layoutActionsContent(colW - 36, false);
            ...
            var cardW = contentW;
            layoutPanel("project", left, y, cardW, 336);
            layoutProjectContent(cardW - 36, true);
            y += 336 + gap;

            layoutPanel("actions", left, y, cardW, 342);
            layoutActionsContent(cardW - 36, true);
            y += 342 + gap;

            layoutPanel("animations", left, y, cardW, 250);
            layoutAnimationsContent(cardW - 36, 176, true);
            y += 250 + gap;

            layoutPanel("console", left, y, cardW, 150);
            layoutConsoleContent(cardW - 36, 76);
            y += 150 + 12;
        }
    ...
    function layoutProjectContent(innerW:Float, compact:Bool):Void {
        ...
        layoutInput("animationJson", 0, 0, innerW, compact);
        layoutInput("atlasJson", 0, rowGap, innerW, compact);
        layoutInput("atlasPng", 0, rowGap * 2, innerW, compact);
        layoutInput("animsXml", 0, rowGap * 3, innerW, compact);
    }

    function layoutActionsContent(innerW:Float, compact:Bool):Void {
        var rowGap = compact ? 64.0 : 74.0;
        layoutInput("animsJson", 0, 0, innerW, compact);
        layoutInput("outputDir", 0, rowGap, innerW, compact);

        if (statusField != null) {
            statusField.x = 0;
            statusField.y = compact ? 134 : 148;
            statusField.width = innerW;
            statusField.height = 22;
        }

        var buttonGap = 10.0;
        var primaryY = compact ? 164.0 : 180.0;
        var secondaryY = compact ? 232.0 : 242.0;
        var primaryW = (innerW - buttonGap) * 0.5;
        layoutButton("loadBtn", 0, primaryY, primaryW, compact ? 52 : 54);
        layoutButton("exportBtn", primaryW + buttonGap, primaryY, primaryW, compact ? 52 : 54);
    ...
    function describeProject():Void {
        syncPathsFromFields();
        var result = Backend.loadProject(paths);
    ...
    function exportProject():Void {
        ...
        var result = Backend.exportProject(paths, chosen, true);
        if (result.log != null && StringTools.trim(result.log) != "") {
            AppLogger.log(result.log);
        }
        ...
        if (statusField != null) statusField.text = 'Exportadas ' + result.filesWritten + ' imagen(es).';
    }
    ...
    function updateSelectionCount():Void {
        ...
        if (statusField != null) statusField.text = text;
